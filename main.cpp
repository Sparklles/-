...
private:
    // 图表视图
    QtCharts::QChartView *taskPieChartView;   // 用于任务完成率饼图
    QtCharts::QChartView *taskBarChartView;   // 用于任务统计的柱状图（按时间聚合）
    QtCharts::QChartView *eventBarChartView;  // 用于事件统计的柱状图（按时间聚合）
    ...
    void setupUI() {
        ...
        // 第二部分：图表区域
        // 左侧显示饼图：任务整体完成率
        taskPieChartView = new QtCharts::QChartView(new QtCharts::QChart(), this);
        // 右侧显示柱状图：任务、事件按时间统计
        taskBarChartView = new QtCharts::QChartView(new QtCharts::QChart(), this);
        eventBarChartView = new QtCharts::QChartView(new QtCharts::QChart(), this);
    ...
    void updateTaskPieChart() {
        ...
        // 更新图表
        QtCharts::QChart *chart = taskPieChartView->chart();
    ...
    void updateTaskBarChart() {
        ...
        QtCharts::QBarSet *completedSet = new QtCharts::QBarSet("已完成");
        QtCharts::QBarSet *incompletedSet = new QtCharts::QBarSet("未完成");
    ...
    void updateEventBarChart() {
        ...
        // 对每个类别创建一个 BarSet
        QMap<QString, QtCharts::QBarSet*> categoryBarSets;
        for (const QString &cat : categories) {
            categoryBarSets[cat] = new QtCharts::QBarSet(cat);
        }
...
private:
    ...
    void setupUI() {
        ...
        detailText->setStyleSheet("background-color: #f0f0f0;");
...
//        reportTypeCombo->addItem("月报");
        ...
        if(dialog.exec() == QDialog::Accepted) {
            ...
            query.addBindValue(QDateTime(reportDate));
